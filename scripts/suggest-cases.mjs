...
check('reason gives both signals when both apply', scoreTrack(newDrake, genres, rap).reason === 'Already has Drake and lots of hip hop');
check('reason is artist-only when the genre is a sliver of the playlist', scoreTrack(newDrake, genres, { ...rap, genreWeights: new Map([['hip hop', 0.02]]) }).reason === 'Already has Drake');
...
// Four micro-genres, one of them a fifth of the playlist: enough to suggest on genre alone
const eclectic = T('n8', A('someone', 'Someone'));
const eclecticGenres = new Map([...genres, ['someone', ['hip hop', 'trap soul', 'cloud rap', 'phonk']]]);
check('a strong single genre match clears the bar without the artist', suggestPlaylists(eclectic, eclecticGenres, profiles)[0]?.id === 'rap');
